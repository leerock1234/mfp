...
public abstract class Heap<A extends Comparable<A>> {
    ...
    public static <A extends Comparable<A>> Heap<A> heap(List<A> list) {
        return list.foldLeft(empty(), t -> t::add);
    }

    @SafeVarargs
    public static <A extends Comparable<A>> Heap<A> heap(A... as) {
        return heap(List.list(as));
    }
}
