...
public abstract class Tree<A extends Comparable<A>> {
    ...
    private Function<List<A>, Function<List<A>, List<A>>> g = a -> b -> a.cons(b);
    ...
    private static class T<A extends Comparable<A>> extends Tree<A> {
        ...
        public List<Integer> pathLengths(int currentDepth, List<Integer> depths) {
            return right.isEmpty() && left.isEmpty()
                    ...
                    : left.pathLengths(currentDepth + 1, depths).cons(right.pathLengths(currentDepth + 1, depths));
        }
        ...
        public List<List<Color>> pathColors(List<Color> currentColorList, List<List<Color>> paths) {
            return right.isEmpty() && left.isEmpty()
                    ...
                    : left.pathColors(currentColorList.cons(color), paths).cons(right.pathColors(currentColorList.cons(color), paths));
        }
